...
public class TaskAllocater {
    ...
    public TaskAllocater(String orignFileFullName, int readTaskNum, int writeTaskNum, int maxLineSize, FileSplitter fileSplitter, BlockingQueue<FileLine> queue) {
    ...
    public List<FileWriteTask> initFileWriteTask() {
        ...
        for (int i = 0; i < this.writeTaskNum; i++) {
            FileWriteTask task = new FileWriteTask(i, this.fileSplitter, this.queue);
            taskList.add(task);
