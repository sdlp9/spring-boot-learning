...
import org.yee.learning.springboot.filehanding.Constants;
...
public class DisruptorMaster extends Master {
    ...
    public void execute() {
        ...
        List<Future> fileWriteFutureList = new ArrayList<>();
        for (FileWriteTask task : fileWriteTasks) {
            Future future = this.fileWritePool.submit(task, task);
            fileWriteFutureList.add(future);
        }

        // start org.yee.learing.springboot.filehanding.disruptor
        disruptor.start();

        // submit FileReadTasks
        List<Future> fileReadFutureList = new ArrayList<>();
        ...
        //close org.yee.learing.springboot.filehanding.disruptor
        this.disruptor.shutdown();

        //check file split result
...
//			BlockingQueue<FileLine> queue = new SynchronousQueue<FileLine>();
            FileWriteTask task = new FileWriteTask(i, this.getFileSplitter(), queue);
